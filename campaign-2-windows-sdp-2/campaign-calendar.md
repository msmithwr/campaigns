# Campaign: Windows EC2 SDP Campaign 2

## 1. Campaign Overview

Windows EC2 SDP Campaign 2 is now a one-month campaign focused on Windows SDP Savings & Modernisation Benefits. It uses the second-month wave from the Windows campaign sequence and runs in July 2026.

## 2. Campaign Timeline (July 2026)

The campaign contains one monthly wave with operational-efficiency messaging, SDP savings calculator promotion, WhatsApp follow-ups, sales calls, and qualified opportunity advancement.

## 3. High-Level Monthly Theme

| Month | Theme | Primary CTA |
|-------|-------|-------------|
| July 2026 | Windows SDP Savings & Modernisation Benefits | Windows-to-AWS TCO Comparison / SDP Savings Calculator Session |

## 4. Detailed Campaign Calendar (Weekly Breakdown)

### JULY 2026 – WINDOWS SDP SAVINGS & MODERNISATION BENEFITS

#### Week 1 (1–7 July)
**Activities:**
* Publish LinkedIn article: "Reducing Windows Infrastructure Costs by 45% Using AWS SDP"
* Launch targeted digital ads for KSA CTOs, IT Operations Managers, Finance Directors, and Procurement teams
* Promote Windows SDP value propositions across LinkedIn and WhatsApp campaigns
* Review Campaign 1 engagement data and prioritise warm Windows accounts

#### Week 2 (8–14 July)
**Outbound:**
* Email 1 to all personas (Windows SDP introduction + immediate cost benefits)
* WhatsApp Message 1

**Sales Activities:**
* Stage 1 Calls using Windows cost-optimisation scripts
* Focus on IT Operations Manager and Procurement Manager conversion
* Qualify accounts for SDP Savings Calculator Sessions

#### Week 3 (15–21 July)
**Outbound:**
* Email 2 to all personas (Windows financial modelling, SDP licensing benefits, vendor simplification)
* WhatsApp Message 2

**Sales Activities:**
* Stage 2 Calls
* Deliver Windows TCO baselines for accounts engaged during Campaign 1
* Deliver SDP Savings Calculator Sessions for qualified prospects

#### Week 4 (22–31 July)
**Outbound:**
* Email 3 (Windows cost-saving case study with SDP benefits)
* WhatsApp Message 3

**Sales Activities:**
* Stage 3 Calls
* Deliver Windows-to-AWS TCO Comparisons (12 targets)
* Windows-qualified pipeline review and opportunity advancement

**KPI Targets:**
* 30% email open rate
* 18% CTR
* 12 Windows TCO Comparisons delivered
* 8 qualified opportunities created

## 5. Integration of WhatsApp, Landing Pages, and Call Scripts

### WhatsApp Messages – Operational Plan

#### Usage Rules
1. Send 48 hours after each email
2. Include the SDP savings angle in each WhatsApp follow-up
3. Keep messages personalised with Windows-specific pain points
4. Use Arabic greetings for KSA market appropriateness

### Landing Pages – Execution Plan

| Persona | Landing Page | CTA |
|---------|--------------|-----|
| CTO / Head of IT | Windows Modernisation | Book Windows-to-AWS TCO Comparison |
| CFO | Windows TCO & SDP Savings | Schedule SDP Savings Calculator Session |
| IT Operations Manager | Windows Operational Excellence | Request Windows Operations Workshop |
| Procurement / Vendor Manager | Windows Licensing Optimisation | Review SDP Savings Options |

### Call Scripts – Execution Plan

| Email | Follow-Up Timing | Call Script Stage |
|-------|------------------|-------------------|
| Email 1 | +48–72 hours | Stage 1 |
| Email 2 | +48 hours | Stage 2 |
| Email 3 | +48 hours | Stage 3 |

## 6. KPIs and Tracking

* Email Open Rate: 30%
* Click-Through Rate: 18%
* Windows TCO Comparisons delivered: 12
* Qualified opportunities created: 8
